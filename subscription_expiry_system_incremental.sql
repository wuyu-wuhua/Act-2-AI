-- 订阅积分系统增量更新脚本
-- 只包含新增的功能，避免重复执行已有的部分
-- 执行前请备份数据库

-- 1. 检查并添加新的订阅相关字段（如果不存在）
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'act_users' AND column_name = 'subscription_status') THEN
        ...
        RAISE NOTICE 'Added subscription_status column to act_users';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'act_users' AND column_name = 'subscription_start_date') THEN
        ...
        RAISE NOTICE 'Added subscription_start_date column to act_users';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'act_users' AND column_name = 'subscription_end_date') THEN
        ...
        RAISE NOTICE 'Added subscription_end_date column to act_users';
    END IF;
    
    -- 检查subscription_cancelled_date字段
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'act_users' AND column_name = 'subscription_cancelled_date') THEN
        ...
        RAISE NOTICE 'Added subscription_cancelled_date column to act_users';
    END IF;
    
    -- 检查subscription_plan_id字段
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'act_users' AND column_name = 'subscription_plan_id') THEN
        ...
        RAISE NOTICE 'Added subscription_plan_id column to act_users';
    END IF;
END $$;
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_act_users_subscription_status') THEN
        CREATE INDEX idx_act_users_subscription_status ON act_users(subscription_status);
        RAISE NOTICE 'Created idx_act_users_subscription_status index';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_act_users_subscription_end_date') THEN
        CREATE INDEX idx_act_users_subscription_end_date ON act_users(subscription_end_date);
        RAISE NOTICE 'Created idx_act_users_subscription_end_date index';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_act_subscription_status_user_id') THEN
        CREATE INDEX idx_act_subscription_status_user_id ON act_subscription_status(user_id);
        RAISE NOTICE 'Created idx_act_subscription_status_user_id index';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_act_subscription_status_status') THEN
        CREATE INDEX idx_act_subscription_status_status ON act_subscription_status(subscription_status);
        RAISE NOTICE 'Created idx_act_subscription_status_status index';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_act_subscription_status_end_date') THEN
        CREATE INDEX idx_act_subscription_status_end_date ON act_subscription_status(subscription_end_date);
        RAISE NOTICE 'Created idx_act_subscription_status_end_date index';
    END IF;
END $$;

-- 4. 创建或更新函数：update_subscription_status
...
-- 6. 创建或更新函数：expire_subscription_credits
...
-- 7. 创建或更新函数：process_expired_subscriptions
...
-- 8. 创建新函数：renew_subscription_credits（新增功能）
...
-- 9. 创建或更新视图：act_expiring_subscriptions
...
-- 10. 创建或更新函数：daily_subscription_cleanup
...
-- 11. 创建或更新函数：add_subscription_credits（支持续期检测）
...
-- 12. 验证更新
SELECT 
    'Subscription expiry system incremental update completed successfully' as status,
